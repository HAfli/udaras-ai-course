...
export function PrintableWorkshopSheets({ className = '' }: { className?: string }) {
  return (
    <section aria-labelledby="printable-sheets-h" className={`rounded-xl2 border border-moss-300 bg-moss-50/60 p-6 sm:p-8 ${className}`}>
      <p id="printable-sheets-h" className="kicker text-moss-600">Printable workshop sheets</p>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        {SHEETS.map(s => (
          <div key={s.id} className="rounded-xl2 border border-ink/10 bg-paper-card p-4">
            <p className="font-display text-base font-semibold leading-snug">{s.title}</p>
            <div className="mt-3 flex flex-wrap gap-x-5 gap-y-1.5 text-sm">
              <a
                href={`#/session/s1/exercises/${s.id}`}
                className="inline-flex items-center gap-1 font-semibold text-moss-700 hover:underline"
              >
